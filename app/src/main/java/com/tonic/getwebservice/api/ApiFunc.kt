...
class ApiFunc {
    ...
    private val apiStrGetJsonCount = baseIP + "OdbcGetValueTestJson"
    ...
    fun getXML(para: HttpXMLGetPara, callback: Callback) {
        Log.e(mTAG, "ApiFunc->getReceipt(xml)")
    ...
    fun getJson(para: HttpJsonGetPara, callback:Callback){
        Log.e(mTAG, "ApiFunc->getReceipt(xml)")
        //postWithParaStrandTimeOut(apiStrGetAssemblyCount, para.x,para.y, callback)
        postWithParaStrandTimeOutJson(apiStrGetJsonCount, Gson().toJson(para), callback)
    }
    private fun postWithParaStrandTimeOutJson(url: String, jsonStr: String, callback: Callback) {
